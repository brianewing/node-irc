    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "PRIVMSG":
                ...
                if ( ctcp = text.match(/^\u0001([a-zA-Z0-9_-]+)( (.+))?\u0001$/) ) {
                    var verb = ctcp[1].toLowerCase(),
                        text = ctcp[3] || null;
                    
                    // could do with some refactoring
                    self.emit('ctcp', from, to, verb, text, message);
                    self.emit('ctcp.' + verb, from, to, text, message);
                    if ( to.match(/^[&#]/) ) {
                        self.emit('ctcp#', from, to, verb, text, message);
                        self.emit('ctcp#.' + verb, from, to, text, message);
                        self.emit('ctcp' + to, from, verb, text, message);
                        self.emit('ctcp' + to + '.' + verb, from, text, message);
                        
                        if ( to != to.toLowerCase() ) {
                            self.emit('ctcp' + to.toLowerCase(), from, verb, text, message);
                            self.emit('ctcp' + to.toLowerCase() + '.' + verb, from, text, message);
                        }
                    }
                    
                    if ( self.opt.debug )
                        util.log('GOT CTCP ' + verb + ' from ' + from + ': ' + text);
                } else {
...
Client.prototype.ctcp = function(target, verb, args) { // {{{
    if (typeof text !== 'undefined') {
        ...
        this.say(target, '\u0001' + verb.toUpperCase() + args + '\u0001');
    }
} // }}}
Client.prototype.ctcpReply = function(target, verb, text) { // {{{
    text = text ? ' ' + text : '';
    this.notice(target, '\u0001' + verb.toUpperCase() + text + '\u0001');
} // }}}
